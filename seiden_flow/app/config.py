...
class Settings:
    ...
    era_poll_seconds: int = 30
    era_critical_delay_minutes: int = 0
    ...
    era_telegram_routes: tuple[str, ...] = ()
    era_email_enabled: bool = False
    era_email_smtp_host: str = ""
    era_email_smtp_port: int = 587
    era_email_username: str = ""
    era_email_password: str = ""
    era_email_from: str = ""
    era_email_to: tuple[str, ...] = ()
    era_email_starttls: bool = True
...
def load_settings() -> Settings:
    ...
    return Settings(
        ...
        tca_enabled=_bool(raw.get("tca_enabled"),True),
        ...
        era_enabled=_bool(raw.get("era_enabled"),True),
        era_poll_seconds=max(10,min(3600,int(raw.get("era_poll_seconds",30)))),
        era_critical_delay_minutes=max(0,min(1440,int(raw.get("era_critical_delay_minutes",0)))),
        ...
        era_telegram_routes=_strings(raw.get("era_telegram_routes",[])),
        era_email_enabled=_bool(raw.get("era_email_enabled"),False),
        era_email_smtp_host=str(raw.get("era_email_smtp_host","") or "").strip(),
        era_email_smtp_port=max(1,min(65535,int(raw.get("era_email_smtp_port",587)))),
        era_email_username=str(raw.get("era_email_username","") or "").strip(),
        era_email_password=str(raw.get("era_email_password","") or ""),
        era_email_from=str(raw.get("era_email_from","") or "").strip(),
        era_email_to=_strings(raw.get("era_email_to",[])),
        era_email_starttls=_bool(raw.get("era_email_starttls"),True),

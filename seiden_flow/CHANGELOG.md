...
- Sincroniza `config.yaml`, `app/version.py`, manifest ITA e footers com a versão publicada.
- Restaura o histórico completo do CHANGELOG que havia sido truncado na 0.22.1.6.
- Corrige README e DOCS para refletirem Linux Host UX e Smart Overview Filters nas releases corretas.
- Mantém os filtros inteligentes e a home capability-aware para `linux_host`.
- Backend, APIs, banco, lifecycle e lógica dos demais módulos permanecem inalterados.

## 0.22.1.7 — ITA 0.3.1.6 · Smart Overview Filters

- Cards de resumo da home passam a funcionar como filtros clicáveis.
- Filtros: Todos/Ativos, Normais, Atenção, Críticos e Sem telemetria.
- Seleção recebe estado visual, `aria-pressed` e persistência por sessão.
- Grade de ativos é filtrada instantaneamente sem alterar as contagens globais da visão.
- Estados vazios recebem mensagem contextual.
- Cards Linux deixam de exibir `Apps 0/0`; passam a priorizar métricas coerentes com `linux_host`.
- Backend, APIs, banco e lifecycle permanecem inalterados.

## 0.22.1.6 — ITA 0.3.1.5 · Linux Host UX

- ITA passa a reconhecer `linux_host` recebido pela Seiden Pulse Linux.
- Detalhe Linux usa `latest_snapshot.telemetry.linux` já exposto pelo Receiver 0.2.3.1.
